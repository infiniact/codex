...
impl CodexMessageProcessor {
    ...
    async fn logout_common(&mut self) -> Result<AuthMode, JSONRPCErrorError> {
        // Cancel any active login attempt.
        {
            let mut guard = self.active_login.lock().await;
            if let Some(active) = guard.take() {
        ...
        self.auth_manager
            ...
            .ok_or_else(|| JSONRPCErrorError {
                code: INTERNAL_ERROR_CODE,
                message: "No auth mode available after logout".to_string(),
                data: None,
            })
    }
    ...
    async fn logout_v2(&mut self, request_id: RequestId) {
        match self.logout_common().await {
            Ok(current_auth_method) => {
                ...
                let payload_v2 = AccountUpdatedNotification {
                    auth_mode: Some(current_auth_method),
                };
                self.outgoing
                    .send_server_notification(ServerNotification::AccountUpdated(payload_v2))
                    .await;
